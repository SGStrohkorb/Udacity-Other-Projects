...
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
C:\textbackslash{}Users\textbackslash{}Sam\textbackslash{}AppData\textbackslash{}Local\textbackslash{}conda\textbackslash{}conda\textbackslash{}envs\textbackslash{}boston\textbackslash{}lib\textbackslash{}site-packages\textbackslash{}sklearn\textbackslash{}cross\_validation.py:44: DeprecationWarning: This module was deprecated in version 0.18 in favor of the model\_selection module into which all the refactored classes and functions are moved. Also note that the interface of the new CV iterators are different from that of this module. This module will be removed in 0.20.
  "This module will be removed in 0.20.", DeprecationWarning)
C:\textbackslash{}Users\textbackslash{}Sam\textbackslash{}AppData\textbackslash{}Local\textbackslash{}conda\textbackslash{}conda\textbackslash{}envs\textbackslash{}boston\textbackslash{}lib\textbackslash{}site-packages\textbackslash{}sklearn\textbackslash{}learning\_curve.py:23: DeprecationWarning: This module was deprecated in version 0.18 in favor of the model\_selection module into which all the functions are moved. This module will be removed in 0.20
  DeprecationWarning)
...
\texttt{prices}. - Store each calculation in their respective variable.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}2}]:} \PY{n}{minimum\PYZus{}price} \PY{o}{=} \PY{n}{prices}\PY{o}{.}\PY{n}{min}\PY{p}{(}\PY{p}{)}
        \PY{n}{maximum\PYZus{}price} \PY{o}{=} \PY{n}{prices}\PY{o}{.}\PY{n}{max}\PY{p}{(}\PY{p}{)}
        \PY{n}{mean\PYZus{}price} \PY{o}{=} \PY{n}{prices}\PY{o}{.}\PY{n}{mean}\PY{p}{(}\PY{p}{)}
        \PY{n}{median\PYZus{}price} \PY{o}{=} \PY{n}{prices}\PY{o}{.}\PY{n}{median}\PY{p}{(}\PY{p}{)}
        \PY{n}{std\PYZus{}price} \PY{o}{=} \PY{n}{prices}\PY{o}{.}\PY{n}{std}\PY{p}{(}\PY{p}{)}
...
\texttt{\textquotesingle{}PTRATIO\textquotesingle{}} value of 15?
    ...
    \textbf{Answer: } For 'RM', I would expect an increase in 'RM' would
corrispond to an increase in 'MEDV'. For 'LSTAT', I would expect an
increase in 'LSTAT' would corrispond to a decrease in 'MEDV'. For
'PTRATIO', I would expect an increase in 'PTRATIO' would corrispond to
decrease in 'MEDV'.
...
to the \texttt{score} variable.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}3}]:} \PY{k+kn}{from} \PY{n+nn}{sklearn}\PY{n+nn}{.}\PY{n+nn}{metrics} \PY{k}{import} \PY{n}{r2\PYZus{}score}
...
the variance in Y is predictable from X.
    ...
    \textbf{Answer:} I would say the model has successfully captured the
variation of the target variable because the R2 score is close to 1,
indicating that 92.3\% of the variance in Y is predictable from X, which
is a high percent.
...
{\color{incolor}In [{\color{incolor}5}]:} \PY{k+kn}{from} \PY{n+nn}{sklearn}\PY{n+nn}{.}\PY{n+nn}{cross\PYZus{}validation} \PY{k}{import} \PY{n}{train\PYZus{}test\PYZus{}split}
        ...
        \PY{n}{random}\PY{o}{.}\PY{n}{seed}\PY{p}{(}\PY{l+m+mi}{72}\PY{p}{)}
        
        \PY{n}{X\PYZus{}train}\PY{p}{,} \PY{n}{X\PYZus{}test}\PY{p}{,} \PY{n}{y\PYZus{}train}\PY{p}{,} \PY{n}{y\PYZus{}test} \PY{o}{=} \PY{n}{train\PYZus{}test\PYZus{}split}\PY{p}{(}\PY{n}{features}\PY{p}{,} \PY{n}{prices}\PY{p}{,} \PY{n}{test\PYZus{}size}\PY{o}{=}\PY{l+m+mf}{0.2}\PY{p}{,} \PY{n}{random\PYZus{}state}\PY{o}{=}\PY{l+m+mi}{72}\PY{p}{)}
...
upon how splits on data is done.
    ...
    \textbf{Answer: } It is beneficial to split the data into soem ratio to
make sure there is enough data to make a successful model that isn't
underfit, but not too much data to create an overfit model
...
curves are converging.
    ...
    \textbf{Answer: } I am going to look at max\_depth = 1. As more points
are added to the training score, it keeps decreasing and seems to be
aproaching 0.42 from above. As more points are added to the testing
score, it first increses to around 0.41 and stays relatively close to
it. More training points would make the training score continue to
converge to the testing score.
...
the tradeoff.
    ...
    \textbf{Answer: } At maximum depth of 1, the model is suffering from
high bias, because the training and validation scores are both
relatively low, but still close together. At maximum depth of 10, the
model is suffering from high variance, because the training score is
very high and the two scores are seperated by a large margin.
...
the fewest assumptions should be selected."
    ...
    \textbf{Answer: } Maximum depth of 3 is the best model because the
training and validation scores are relatively close to
...
a parameter in a model that can be optimized using this approach.
    ...
    \textbf{Answer: } The grid search technique is a way of varying multiple
different parameters through all combinations of those parameters to
find which set of parameters produces the highest score in some scoring
function for some machine learning algorithm. This can be applied to a
learning algorithm to optimize it by automizing the process of
optimization. One could manually go through and change parameters until
they found the best combination, but grid search optimizes that process,
...
for your answer.
    ...
    \textbf{Answer: } K-fold cross-validation, the data is split up into 'k'
buckets (or groups), with one of the buckets as the testing set and the
other buckets as the training set. This is done 'k' times to allow for
optimum use of the data set. This technique benefits grid search because
it helps make sure that the model truly fits all of the data and not
just one training and testing set, preventing overfitting.
...
\texttt{\textquotesingle{}grid\textquotesingle{}} variable.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}8}]:} \PY{k+kn}{from} \PY{n+nn}{sklearn}\PY{n+nn}{.}\PY{n+nn}{tree} \PY{k}{import} \PY{n}{DecisionTreeRegressor}
        ...
        \PY{l+s+sd}{        decision tree regressor trained on the input data [X, y]. \PYZdq{}\PYZdq{}\PYZdq{}}
            ...
            \PY{n}{cv\PYZus{}sets} \PY{o}{=} \PY{n}{ShuffleSplit}\PY{p}{(}\PY{n}{X}\PY{o}{.}\PY{n}{shape}\PY{p}{[}\PY{l+m+mi}{0}\PY{p}{]}\PY{p}{,} \PY{n}{n\PYZus{}iter} \PY{o}{=} \PY{l+m+mi}{10}\PY{p}{,} \PY{n}{test\PYZus{}size} \PY{o}{=} \PY{l+m+mf}{0.20}\PY{p}{,} \PY{n}{random\PYZus{}state} \PY{o}{=} \PY{l+m+mi}{72}\PY{p}{)}
            \PY{n}{regressor} \PY{o}{=} \PY{n}{DecisionTreeRegressor}\PY{p}{(}\PY{n}{random\PYZus{}state}\PY{o}{=}\PY{l+m+mi}{72}\PY{p}{)}
            ...
            \PY{n}{grid} \PY{o}{=} \PY{n}{GridSearchCV}\PY{p}{(}\PY{n}{regressor}\PY{p}{,} \PY{n}{params}\PY{p}{,} \PY{n}{scoring}\PY{o}{=}\PY{n}{scoring\PYZus{}fnc}\PY{p}{,} \PY{n}{cv}\PY{o}{=}\PY{n}{cv\PYZus{}sets}\PY{p}{)}
...
Predicted selling price for Client 3's home: \$938,053.85
    ...
    \textbf{Answer: } Based on the model, for all the Clients, I would use
the prices predicted by the model. For Client 1, 414,400.00 is a
resonable price for the house, because it is near both the mean and
median and easily inside one standard deviation of the mean. For Client
2, 237,951.72 is a resonable price for the house, because it is about
one and a half standard deviations away from the mean, so it isn't the
worst house on the market, but it is down there. For Client 3,
938,053.85 is a resonable price for the house, because it is quite a
large home with low poverty and good schools. It is almost three
standard deviations away from the mean, so there are not many other
homes like it.
...
the entire neighborhood?
    ...
    \textbf{Answer: } This model is not suitable for real-world use. First,
the data is based off of one city, so applying this model to any other
city or town would not return accurate results, because the cost of
land, taxes, standards of living, etc. play into how much a house costs.
Second, the model does not take into account other factors, like roof
quality, appliances, pools, carpetting v. hardwood, granate counter
tops, and other qualities. These qualities, once added together, can
sway the price of the house quite a bit. Lastly, this model does not
account for the housing market. When the market has too many buyers or
too many sellers, the prices that houses are sold from can very by quite
a bit, causing inconsistencies in the data. Overall, if this model was
used in Boston, it would give a seller a general sense of how much they
could sell their house for, but definately not a final price.
